...
enable_analog_fail:
	gpio_request(ELITE_GPIO_V_CAM_D1V2_EN, "CAM_D1V2_EN");
...
static int elite_rawchip_vreg_off(void)
{
	...
	if (rc < 0) {
		...
		goto elite_rawchip_vreg_off_fail;
	}
...
static int elite_mt9v113_vreg_on(void)
{
	int rc;
	...
	rc = gpio_request(ELITE_GPIO_V_LCMIO_1V8_EN, "CAM_D1V8_EN");
	pr_info("elite_mt9v113_vreg_on %d 1v8\n", ELITE_GPIO_V_LCMIO_1V8_EN);
	if (rc < 0) {
		pr_err("%s:GPIO_CAM_D1V8_EN gpio %d request failed, rc=%d\n", __func__,  ELITE_GPIO_V_LCMIO_1V8_EN, rc);
		goto init_fail;
	}
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9v113_data = {
	...
	.vcm_pwd	= 0,
	.vcm_enable	= 1,
	.camera_power_on = elite_mt9v113_vreg_on,
	.camera_power_off = elite_mt9v113_vreg_off,
